...
public class ViewFicheHorsForfait extends JFrame {
	...
	private JLabel getLblTitre() {
		if (lblTitre == null) {
			lblTitre = new JLabel("Nombre de fiches hors forfait par région et mois");
	...
	private JTable getTableFrais() {
		if (tableFrais == null) {
			...
			String[] entetes = { "Nom", "Prénom", "Nombre de fiches" };
	...
	private void mettreAJourTableau() {
		...
		for (Utilisateur u : tousLesUtilisateurs) {
			if (u.getRegion() != null && u.getRegion().equals(regionSelect)) {
				int nbFrais = AccesData.getNbFicheHorsForfaitParMois(u.getIdUtilisateur(), moisFiltre);

				if (nbFrais > 0) {
					modele.addRow(new Object[] { u.getNom(), u.getPrenom(), nbFrais });
				}
			}
		}
	}
