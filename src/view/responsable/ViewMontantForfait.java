...
public class ViewMontantForfait extends JFrame {
	...
	private JLabel getLblTitre() {
		if (lblTitre == null) {
			lblTitre = new JLabel("Montant total des frais forfait par région et mois");
	...
	private void mettreAJourTableau() {
		...
		for (Utilisateur u : tousLesUtilisateurs) {
			if (u.getRegion() != null && u.getRegion().equals(regionSelect)) {
				double montant = AccesData.getMontantFraisForfaitParMois(u.getIdUtilisateur(), moisFiltre);
