...
/**
 * Écran de la moyenne des frais hors forfait par région.
 *
 * @author Matthieu CAMPAGNA
 */
public class ViewMoyenneHorsForfait extends JFrame {
	...
	private JLabel getLblTitre() {
		if (lblTitre == null) {
			lblTitre = new JLabel("Moyenne des frais hors forfait par région");
	...
	private JTable getTableFrais() {
		if (tableFrais == null) {
			...
			String[] entetes = { "Région", "Nombre de visiteurs", "Moyenne Frais Hors Forfait (€)" };
	...
	private void mettreAJourTableau() {
		...
		List<Object[]> moyenneRegion = AccesData.getMoyenneFraisHorsForfaitParRegion(moisFiltre);
