...
/**
 * Écran de suppression d'un visiteur avec affichage de sa fiche (RH).
 *
 * @author Matthieu CAMPAGNA
 */
public class ViewSupprimerRh extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ViewSupprimerRh frame = new ViewSupprimerRh();
	...
	private JLabel getLblTitre() {
		if (lblTitre == null) {
			lblTitre = new JLabel("Supprimer le visiteur");
	...
	private JButton getBtnSupprimer() {
		if (btnSupprimer == null) {
			...
			btnSupprimer.setBounds(450, 480, 120, 30);
			btnSupprimer.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (vide == true) {
						...
						boolean ok = AccesData.deleteUtilisateur(u);
						...
						if (ok == true) {
							...
							JOptionPane.showMessageDialog(null,
									"Erreur : Impossible de supprimer le visiteur.");
						}
					}
				}
			});
